            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Service ID: <code className="bg-slate-100 text-slate-600 rounded px-1 font-mono">{selectedService.id}</code> • Host Port:{" "}
                  <code className="bg-slate-100 text-slate-600 rounded px-1 font-mono">{selectedService.port}</code>
                </p>
              </div>
            </div>
            <StatusBadge
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-xs">
            <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-2">
              ...
              <div className="space-y-1 text-slate-700">
                <div>Health Check: <code className="bg-slate-100 text-slate-600 rounded px-1">GET /health</code></div>
                <div>API Info: <code className="bg-slate-100 text-slate-600 rounded px-1">GET /api/v1/</code></div>
                <div>Metrics: <code className="bg-slate-100 text-slate-600 rounded px-1">GET /metrics</code></div>
              </div>
            </div>
                ...
                <div className="space-y-1">
                  {selectedService.dependencies.map((dep) => (
                    <div key={dep} className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-amber-700">HTTP REST</span>
                    </div>
                  ))}
                </div>
              ) : (
            ...
            <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-2">
              ...
              <div className="space-y-1">
                <div>Availability: <span className="font-semibold text-slate-900">{selectedService.availability}%</span></div>
                <div>Latency p95: <span className="font-semibold text-slate-900">{selectedService.p95Latency}ms</span></div>
                <div>CPU Load: <span className="font-semibold text-slate-900">{selectedService.cpu}%</span></div>
              </div>
            </div>
          </div>
        </div>
