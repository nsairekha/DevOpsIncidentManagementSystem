"""Self-contained structured JSON request logging for the user-service.

The user-service runs independently from the backend, so it carries its own
copy of the request-ID middleware instead of importing backend code. Every
HTTP request is assigned a unique request ID, is timed, and is logged on
completion as a single JSON line containing ``timestamp``, ``service``,
``method``, ``endpoint``, ``status_code``, ``request_id`` and
``response_time_ms``. The request ID is returned in the ``X-Request-ID``
response header.
"""
